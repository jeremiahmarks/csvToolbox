...
# @Last Modified time: 2015-06-22 15:44:09
import HTMLParser
...
class Product(object):
    ...
    def __eq__(self, other):
        if self.getid() == other.getid():
            return True
        # catsmatch =  len(self.categories.keys())>0 is
        # len(other.categories.keys())>0   # Basically,
        ...
        if self.ProductName and other.ProductName:
            namesmatch = bool(self.ProductName.lower().strip(' \n')
    ...
    def getPublicPage(self, appname):
        if self.Id is not None:
            return "https://" + server.infusionsoftapp + \
                ".infusionsoft.com/app/storeFront/showProductDetail?productId="\
                + str(self.Id)
        else:
    ...
    def getInternalPage(self):
        if self.Id is not None:
            return "https://" + server.infusionsoftapp +\
                ".infusionsoft.com/app/product/manageProduct?productId="\
                + str(self.Id)
        else:
            return "I do not seem to have an ID."
...
class ProductManager(object):
    ...
    def sortItems(self):
        ...
        for eachobject in self.allObjects:
            ...
            self.productsbyname[html_parser.unescape(
                eachobject.ProductName)] = eachobject
    ...
    def getObject(self, aProductValues):
        if "Id" in aProductValues.keys():
            if aProductValues["Id"] in self.productsbyid.keys():
                ...
                matchingrecords = self.server.getMatchingRecords("Product",
                                                                 aProductValues
                                                                 )
                if len(matchingrecords) > 0:
                    ...
                    self.productsbyname[
                        html_parser.unescape(thisProduct.ProductName)
                    ] = thisProduct
