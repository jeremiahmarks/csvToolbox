...
# @Last Modified time: 2015-06-21 23:24:59
...
class ProductCategoryManager(object):
    ...
    def sortItems(self):
        ...
        for eachobject in self.allObjects:
            ...
            if eachobject.CategoryDisplayName not in self.productCatsbyName.keys():
                self.productCatsbyName[eachobject.CategoryDisplayName]={}
            self.productCatsbyName[eachobject.CategoryDisplayName][eachobject.Id]=eachobject
            if eachobject.ParentId not in [None, 0]:
                if eachobject.ParentId not in self.productCatsbyParentId.keys():
                    self.productCatsbyParentId[eachobject.ParentId]={}
                self.productCatsbyParentId[eachobject.ParentId][eachobject.CategoryDisplayName]=eachobject
    ...
    def getObject(self, aProductCategoryValues):
        if "ParentId" not in aProductCategoryValues or aProductCategoryValues["ParentId"] is None:
        ...
        if "Id" in aProductCategoryValues.keys():
            if aProductCategoryValues["Id"] in self.productCatsbyID.keys():
                ...
                matchingrecords=self.server.getMatchingRecords("ProductCategory", aProductCategoryValues)
                if len(matchingrecords)>0:
                    ...
                    if thisProductCategory.CategoryDisplayName not in self.productCatsbyName.keys():
                        self.productCatsbyName[thisProductCategory.CategoryDisplayName]={}
                    self.productCatsbyName[thisProductCategory.CategoryDisplayName][thisProductCategory.Id]=thisProductCategory
                    thisProductCategory.register()
                    return thisProductCategory
                else:
                    thisid=self.server.createNewRecord("ProductCategory", aProductCategoryValues)
            ...
            if not len(aProductCategoryValues["CategoryDisplayName"].strip(' \n'))>0:
                return "This is an invalid CatName name"
            else:
                if aProductCategoryValues["CategoryDisplayName"] in self.productCatsbyName.keys():
